...
describe('BrxEndpointService', () => {
  ...
  describe('buildSuggestionUrl', () => {
    it.skip('should match suggestion url with default parameters', () => {
      ...
      // eslint-disable-next-line max-len
      const resultUrl = `${mockEnvConfig.smSuggestionEndPoint}v1/suggest/?account_id=test-accountId&auth_key=test-authKey&request_id=1703199600000&_br_uid_2=&url=http%3A%2F%2Flocalhost%2F&ref_url=&request_type=suggest&rsp_fmt=v2&q=&domain_key=test-domainKey`;
    ...
    it.skip('should match suggestion url with catalog views and query', () => {
      ...
      // eslint-disable-next-line max-len
      const resultUrl = `${mockEnvConfig.smSuggestionEndPoint}v2/suggest/?account_id=test-accountId&auth_key=test-authKey&request_id=1703199600000&_br_uid_2=&url=http%3A%2F%2Flocalhost%2F&ref_url=&request_type=suggest&rsp_fmt=v2&q=query&catalog_views=catalogViews&rsp_fmt=v2`;
  ...
  describe('buildPathwaysandRecommendationsUrl', () => {
    ...
    it.skip('should match pathwaysandRecommendations url when is in preview', () => {
      ...
      // eslint-disable-next-line max-len
      const resultUrl = `${mockEnvConfig.smEndPoint}v2/widgets/widgetType/widgetId?account_id=test-accountId&view_id=&domain_key=test-domainKey&request_id=1703199600000&_br_uid_2=uid%253D0000000000000%253Av%253D11.5%253Ats%253D1428617911187%253Ahc%253D55&item_ids=id%3DtestId%3B&cat_id=categoryId&query=queryParam&url=http%3A%2F%2Flocalhost%2F&ref_url=&rows=12&fields=pid%2Ctitle%2Cbrand%2Cprice%2Csale_price%2Cpromotions%2Cthumb_image%2Csku_thumb_images%2Csku_swatch_images%2Csku_color_group%2Curl%2Cprice_range%2Csale_price_range%2Cdescription%2Cis_live%2Cscore`;
    ...
    it.skip('should match pathwaysandRecommendations url when is not in preview', () => {
      ...
      // eslint-disable-next-line max-len
      const resultUrl = `${mockEnvConfig.smEndPoint}v2/widgets/widgetType/widgetId?account_id=test-accountId&view_id=&domain_key=test-domainKey&request_id=1703199600000&_br_uid_2=&item_ids=id%3DtestId%3B&cat_id=&query=&url=http%3A%2F%2Flocalhost%2F&ref_url=&rows=12&fields=pid%2Ctitle%2Cbrand%2Cprice%2Csale_price%2Cpromotions%2Cthumb_image%2Csku_thumb_images%2Csku_swatch_images%2Csku_color_group%2Curl%2Cprice_range%2Csale_price_range%2Cdescription%2Cis_live%2Cscore`;
